...
def _load_secrets() -> dict:
    ...
    try:
        ...
        LOGGER.exception("Не удалось прочитать secrets.json.")
        return {}
...
def _secret_get(key: str, env_name: str | None = None) -> str:
    # Переменная окружения имеет приоритет над локальным secrets.json.
    if env_name:
        value = os.getenv(env_name, "").strip()
        if value:
            return value
    value = _load_secrets().get(key, "")
...
def _secret_delete(key: str) -> None:
    secrets = _load_secrets()
    if key in secrets:
...
def api_token_present() -> bool:
    return bool(_secret_get("marketapp_api_token", "MARKETAPP_API_TOKEN"))
...
def _client(auto_pay: bool = False):
    ...
    token = _secret_get("marketapp_api_token", "MARKETAPP_API_TOKEN")
    if not token:
...
def _diag_kb():
    from telebot.types import InlineKeyboardMarkup as K, InlineKeyboardButton as B
    kb = K()
    kb.add(B("🔑 Ввести / заменить API Token", callback_data=f"{CB}:set_api_token"))
    if api_token_present():
        kb.add(B("🗑 Удалить сохранённый API Token", callback_data=f"{CB}:delete_api_token"))
...
def init_telegram(cardinal, *args):
    ...
    def input_handler(message):
        ...
        if key == "marketapp_api_token":
            try:
                if len(raw_text) < 8:
                    raise ValueError("Токен выглядит слишком коротким.")
                _secret_set("marketapp_api_token", raw_text)
                tg.clear_state(message.chat.id, message.from_user.id, True)
                # Best effort: убираем секрет из истории Telegram-чата.
                try:
                    bot.delete_message(message.chat.id, message.id)
                except Exception:
                    LOGGER.debug("Не удалось удалить сообщение с API token из Telegram.", exc_info=True)
                bot.send_message(
                    message.chat.id,
                    "✅ MarketApp API Token сохранён. Значение скрыто. Теперь можно открыть «Диагностика» "
                    "и нажать «Проверить MarketApp API».",
                    reply_markup=K().add(B("🩺 К диагностике", callback_data=f"{CB}:diag"))
                )
            except Exception as exc:
                bot.reply_to(message, "❌ Не удалось сохранить токен: " + html.escape(str(exc)))
            return
    ...
    def ask_api_token(call):
        _remember_admin(call.message.chat.id)
        msg = bot.send_message(
            call.message.chat.id,
            "🔑 <b>MarketApp API Token</b>\n\n"
            "Отправьте токен следующим сообщением. Сообщение с токеном будет удалено из чата "
            "сразу после сохранения. Сам токен в интерфейсе и логах не показывается."
        )
        tg.set_state(call.message.chat.id, msg.id, call.from_user.id, STATE_INPUT_PREFIX + "marketapp_api_token")
        bot.answer_callback_query(call.id)

    def delete_api_token(call):
        # Удаляем только локально сохранённый токен. Переменную окружения удалить из Telegram нельзя.
        _secret_delete("marketapp_api_token")
        if os.getenv("MARKETAPP_API_TOKEN", "").strip():
            extra = "⚠️ Локальный токен удалён, но MARKETAPP_API_TOKEN всё ещё задан в окружении Cardinal."
        else:
            extra = "✅ Сохранённый API Token удалён."
    ...
    tg.cbq_handler(ask_api_token, lambda c: c.data == f"{CB}:set_api_token")
    tg.cbq_handler(delete_api_token, lambda c: c.data == f"{CB}:delete_api_token")
